...
Current local, LocalTunnel, direct IPv6, and relay profile in `1.0.5`:
...
Safari and some locked-down browser contexts do not expose Web Crypto on plain HTTP origins such as `http://172.20.10.8:17865` or `http://[public-ipv6]:17865`. For local mode and direct IPv6 mode, the phone UI can fall back to authenticated plaintext polling on `/api/local/*` when Web Crypto is unavailable. That fallback requires the session id and pairing verifier from the QR/Copy URL fragment on every request. In local mode it is restricted to local-network clients; in direct IPv6 mode it is allowed only because the user explicitly chose the experimental public-IPv6 route. It is not encrypted and should be treated as compatibility behavior, not as E2EE.
...
| Direct IPv6 with Web Crypto | AES-256-GCM encrypted frames for state, commands, and slide assets. | Ephemeral P-256 ECDH plus pairing verifier; direct HTTP requests must also carry the session id. | Public reachability and firewall exposure must be handled carefully. |
| Direct IPv6 compatibility | Plaintext state, commands, and slide assets. | Every `/api/local/*` request must include the session id and pairing verifier headers. | Public-path observers can see traffic; use only for experimental testing when Web Crypto is unavailable. |
...
For `1.0.5`, the supported relay trust model is:

- self-host the published Python relay bundle or deploy the Cloudflare relay from the relay documentation
...
- Safari local and direct-IPv6 compatibility can use authenticated plaintext `/api/local/*` polling only when the browser does not expose Web Crypto
- LocalTunnel and relay mode still require Web Crypto and fail closed when encrypted transport is unavailable
