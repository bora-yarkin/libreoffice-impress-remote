...
This file tracks the `0.7.6` project snapshot: what already ships today and what is still planned next.

For milestone order and upstream strategy, see `docs/roadmap.md`.

Current product direction: local mode is the default path, including same-Wi-Fi and many hotspot setups. One complete OXT ships local, LocalTunnel, direct IPv6, and relay mode support, with matching relay/documentation bundles embedded for export from LibreOffice.

## Planned

### 1. Test And Prove Every Feature
...
### 2. Release And Maintenance Readiness
...
### 3. LibreOffice Upstream Optimization
...
- LibreOffice-owned settings: Remote Settings owns the mode selector, relay configuration, runtime status, manual-link backup, relay/documentation resource export, issue reporting, and help.
- QR-first pairing: Start Remote opens a QR pairing popup, closes after a phone connects, and keeps the manual link in Remote Settings as a backup.
...
- In-product guidance: LibreOffice explains same-Wi-Fi, phone-hotspot, LocalTunnel fallback, direct IPv6, relay, and manual-link usage inside Start Remote, Remote Settings, and Help flows.
...
- Runtime feedback: The phone UI includes a presentation timer, reconnect/offline panel, retry/reload actions, and focus/accessibility polish without installable PWA shell behavior.
